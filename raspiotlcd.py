...
NowTemp = 0.0    # Now Temprature
MScount = 0      # MotionSensor counter
LoopCount = 1    # Loop counter (1sec x 30 = 30sec)
SendCount = 0    # Send counter (0..999)
...
def lcdline(n):
	if n == 0:
		bus.write_byte_data(0x3e, 0x00, 0x80)
	else:
...
try:
	while True:
		...
		if LoopCount == 60:
			readData()
			sndmsg = "node raspiot.js %03d:%2.1f:%d" \
					% (SendCount, NowTemp, MScount)
			print sndmsg
			os.system(sndmsg)
			MScount = 0
			LoopCount = 1
